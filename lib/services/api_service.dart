...
class ApiService {
  ...
  Future<void> _ensureVietnamSymbolMap() async {
    ...
    try {
      _logger.logApiCall('GET', uri.toString());
      final http.Response response = await _client.get(uri);
      _logger.logApiResponse(uri.toString(), statusCode: response.statusCode, body: response.body);
      ...
      _logger.logApiError(uri.toString(), error: error, stackTrace: stackTrace);
    }
  ...
  Future<List<StockPricePoint>> fetchIntradayPrices(
    ...
    return _retryHelper.execute(
      ...
      label: 'fetchIntradayPrices($displaySymbol)',
    );
  }
  ...
  Future<List<StockPricePoint>> fetchHistoricalPrices(
    ...
    return _retryHelper.execute(
      ...
      label: 'fetchHistoricalPrices($displaySymbol)',
    );
  }
  ...
  Future<PortfolioSummary> fetchPortfolio() async {
    ...
    try {
      ...
      _logger.info('Portfolio fetched successfully: ${entries.length} entries');
      return PortfolioSummary(entries: entries);
    } catch (error, stackTrace) {
      _logger.error('Failed to fetch portfolio', error, stackTrace);
      rethrow;
    }
  }
  ...
  Future<UserProfile> fetchUserProfile() async {
    ...
    try {
      ...
      _logger.info('User profile fetched: ${profile.fullName}');
      return profile;
    } catch (error, stackTrace) {
      _logger.error('Failed to fetch user profile', error, stackTrace);
      rethrow;
    }
  }

  Future<UserProfile> updateUserProfile(UserProfile profile) async {
    _logger.info('Updating user profile: ${profile.fullName}');
    try {
      _logger.info('User profile updated successfully');
      return profile;
    } catch (error, stackTrace) {
      _logger.error('Failed to update user profile', error, stackTrace);
      rethrow;
    }
  }
}
